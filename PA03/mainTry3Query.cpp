...
cout<<"TEST"<<endl;
    ...
    for(int j=1204; j<databaseCombined.size()&&correct < 3; j++)
    {
        ...
        {
            /*cout<<j-1204<<endl;
            namedWindow("eigenvector"+char(correct+'A'), CV_WINDOW_AUTOSIZE );
            moveWindow("eigenvector"+char(correct+'A'), 60+100*correct, 60);
            imshow("eigenvector"+char(correct+'A'),databaseCombined.at(j));
            correct++;
            waitKey(0);*/
        }
        else
