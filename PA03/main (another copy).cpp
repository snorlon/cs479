...
Mat norm_0_255(const Mat& src) {
    ...
    Mat dst;
    cv::normalize(src, dst, 0, 1, NORM_MINMAX, CV_32FC1);
...
cout<<maxFaces<<endl;
        ...
        //convert pixels
        for(int i=0; i<tempFaceMat.rows; i++)
            for(int j=0; j<tempFaceMat.cols; j++)
            {
                tempFaceMat.at<float>(i,j) /= 255.0;
            }
        ...
        for(int i=0; i<tempFaceMat.rows; i++)
            for(int j=0; j<tempFaceMat.cols; j++)
            {
                avgFaceMat.at<float>(i,j) /= 255.0;
                cout<<avgFaceMat.at<float>(i,j)<<endl;
            }

        //for each i in outputMatrix, it should be the multiplied product of every single image by the test image
        for(int i=0; i<maxFaces; i++)
        {
            //modify the eigenface
            Mat newEigenFace;
            eigenfaces.at(i).convertTo(newEigenFace, CV_32FC1);

            //convert pixels
            for(int m=0; m<newEigenFace.rows; m++)
                for(int j=0; j<newEigenFace.cols; j++)
                {
                    newEigenFace.at<float>(m,j) /= 255.0;
                }

            for(int j=0; j<testImage.rows; j++)
                for(int k=0; k<testImage.cols; k++)
                {
                    eigenvector_coefficients[i] += (newEigenFace.at<float>(j,k)) * (tempFaceMat.at<float>(j,k) - avgFaceMat.at<float>(j,k));
                }
        }
        ...
        for(int i=0; i<testImage.rows; i++)
            for(int j=0; j<testImage.cols; j++)
            {
                uclidean += pow(newImage.at<float>(i,j) - databaseRaw.at(imgIndex).at<uchar>(i,j),2);
                //cout<<(newImage.at<float>(i,j) - databaseRaw.at(imgIndex).at<uchar>(i,j))<<endl;
            }
                
        uclidean = sqrt(uclidean);

        cout<<uclidean<<endl;

        //float * testData1D = (float*)tempFaceMat.data;

        namedWindow("eigenvectord", CV_WINDOW_AUTOSIZE );
        moveWindow("eigenvectord", 360, 60);
        imshow("eigenvectord",avgFaceMat);
        ...
        imshow("eigenvectorb",tempFaceMat);
